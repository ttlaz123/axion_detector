...
fnames = ['aligned_form_factor.txt'] + [f'dx{distances[i]}um_form_factor.txt' for i in range(len(distances))]
header = [6] + [1]*len(distances)

Cmaxs = np.zeros(len(fnames)) # in.. you know...
...
plt.figure()
plt.title("Form Factor Changes with Misalignment")
plt.ylabel("Form Factor")
plt.xlabel(f"{DoF} Displacement (mm if linear, deg if angluar)")
plt.plot(dists, Cmaxs, 'k')
plt.figure()
plt.title("Check this to see if the white line jumps peaks")
plt.imshow(full_Cs, aspect='auto', interpolation='none', cmap="viridis_r")
plt.colorbar()
plt.show()
